...
| `export_ciso_poam` | reads `out/ciso-assistant/` + `out/poam/` + `out/simplerisk/` | SCOPE-gated paths. `posted` false unless something was actually sent; this tool lists files only. Conductor `http` always false. Does not invent owner/due |
...
| Refusal cases | no SCOPE file / unreadable (`SCOPE_MISSING`); unsigned or attestation hash invalid (`SCOPE_UNSIGNED`); expired engagement window (`SCOPE_EXPIRED`); requested target outside authorized SCOPE (`SCOPE_TARGET`); live + estate other than lab / lab-estate (`LIVE_ESTATE`); live + target outside DESKTOP lab-estate networks `172.28.10.0/24` / `172.28.11.0/24` / `172.31.250.0/24` (`LIVE_LAB_NET`) |
...
`172.28.10.0/24` (labnet), `172.28.11.0/24` (labnet2), and
`172.31.250.0/24` (misconfig overlay internal network) **and** the signed
SCOPE. labnet / labnet2 are declared in DESKTOP
`lab-estate\docker-compose.yml` (compose project `evergreen-lab`);
`172.31.250.0/24` is the misconfig overlay under `lab-estate\misconfig\`.
Confirm with `docker network inspect evergreen-lab_labnet
evergreen-lab_labnet2` (and the misconfig overlay's network) once Docker
Desktop is running; they are not yet runtime-verified. Not invented client
nets. `cli_twin` reflects live vs fixture. Never reseeds
