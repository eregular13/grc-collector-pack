...
LEAK_GET_PATHS = ("/.git/HEAD", "/listing/", "/.env")
...
def describe(scope: Scope, batch: list[str]) -> dict[str, Any]:
    ...
    return {
        ...
        "note": (
            ...
            "allowlisted same-origin GET /.git/HEAD /listing/ /.env and HEAD /cookie /cors only. "
...
def execute(scope: Scope, batch: list[str], timeout: int | None = None) -> dict[str, Any]:
    """HEAD on named URLs; allowlisted same-origin GET /.git/HEAD /listing/ /.env and HEAD /cookie /cors. Never CIDR. Never shell=True."""
