...
**Delta (cycle 26):** External stage is **plan-only** in the graph (after deepen, before ingest). No live curl/testssl from orchestrate. SCOPE refuses CIDR / wildcards / `0.0.0.0/0` in `external:`; named hosts and https URLs allowed. External SLOTS `will_run=false`. `make dropbox-external` stays DEMO fixtures → `in/easm/`. Catalog unchanged (111 / 32 / 30). Layer C untouched.
