...
**Delta (cycle 25):** Farm ↔ orchestrator. Plan lists per-stage SLOTS (`allow_tools ∩ wired invoke`). `--live` discover uses PATH invoke only; deepen uses deepen invoke on discover-live / `deepen_hosts`. nuclei/openvas never subprocess. Catalog unchanged (111 / 32 / 30). Layer C untouched.
