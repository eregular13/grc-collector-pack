...
Three layers (see `ARCHITECTURE.md`): **A** BYO tools on the box under SCOPE — private `farm/` layout (slots + isolation volumes; PATH / bind-mount / Reid-built tags; not Hub soup) · **B** orchestrator = brakes (quiet discover → gated deepen → destroy → ingest) · **C** existing 10 containers parse `in/` only. Layer B feeds Layer C via `in/`; it does not turn collectors into live scanners. “100 tools” = parser file families, not 100 compose binaries. Hexstrike is a UX pattern only (`HEXSTRIKE.md`) — no exploit-chain, no vendor submodule.
...
**Delta (cycle 17):** `farm/` private operator layout. Allowlisted slots (nmap, nessus, testssl, curl, lynis, hardeningkitty-export, prowler, maester, …) as adapters — binaries not vendored. Farm Dockerfile/compose stay scanner-free under the same static asserts. Integrity stop: farm is private. Layer C untouched.
