...
Three layers (see `ARCHITECTURE.md`): **A** BYO tool farm under SCOPE — private `farm/` catalog (48 slots, 13 wired adapters; PATH / bind-mount / Reid-built tags; not Hub soup) · **B** orchestrator = brakes (`plan → shard → discover → destroy → deepen → destroy → ingest → grc_export`) plus stdio MCP conductor · **C** existing 10 containers parse `in/` only. Layer B feeds Layer C via `in/`; it does not turn collectors into live scanners. “100 tools” = parser file families, not 100 compose binaries. Hexstrike is a UX pattern only (`HEXSTRIKE.md`) — no exploit-chain, no vendor submodule.
...
**Delta (cycle 18):** Private farm catalog 48 slots / 13 wired adapters (plan-only + PATH stubs). Orchestrator graph through grc_export. Conductor: 8 SCOPE-gated tools; JSON-RPC invokes plan/status/farm_slots. Compose discover/deepen/ingest workers on an internal network. LICENSE-LOCK names stay file-drop. Layer C untouched.
