...
pytest **126**. `demo: true` on dropbox-lab is the DEMO overlay stamp, not a client estate. Orchestrator on this VM is **plan-only** (no Nmap/Nessus): 3 /24 shards, 2 deepen batches, workers destroyed. `python3 -m dropbox status` shows stage graph, last integrity stop (`not on PATH — plan only`), and allow_tools ∩ PATH (curl present; nmap/nessus missing).
