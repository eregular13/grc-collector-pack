...
def test_prove_ciso_pack_drop_and_honeypot_to_sor(tmp_path: Path) -> None:
    ...
    assert (Path(stamp["in_dir"]) / "nmap" / "pack_drop" / "fping" / "assets.jsonl").is_file()
    ...
    braa_findings = (
        Path(stamp["in_dir"]) / "nmap" / "pack_drop" / "braa" / "findings.jsonl"
    ).read_text(encoding="utf-8")
    assert "snmp_community_observed" in braa_findings or "sysdescr_observed" in braa_findings
    assert "oid_observed" in braa_findings or "sysdescr_observed" in braa_findings
    assert '"claim":"open_port_observed"' not in braa_findings
    assert '"claim": "open_port_observed"' not in braa_findings
...
def test_fixture_banners_are_sample_not_client() -> None:
    ...
    whatweb_meta = (ROOT / "fixtures" / "pack_drop" / "whatweb" / "meta.json").read_text(
        encoding="utf-8"
    )
    whatweb_note = (
        ROOT / "fixtures" / "pack_drop" / "whatweb" / "evidence" / "note.md"
    ).read_text(encoding="utf-8")
    ...
    fping_meta = (ROOT / "fixtures" / "pack_drop" / "fping" / "meta.json").read_text(
        encoding="utf-8"
    )
    fping_note = (
        ROOT / "fixtures" / "pack_drop" / "fping" / "evidence" / "note.md"
    ).read_text(encoding="utf-8")
    fping_sample = (ROOT / "fixtures" / "pack_drop" / "fping" / "SAMPLE.txt").read_text(
        encoding="utf-8"
    )
    naabu_meta = (ROOT / "fixtures" / "pack_drop" / "naabu" / "meta.json").read_text(
        encoding="utf-8"
    )
    naabu_note = (
        ROOT / "fixtures" / "pack_drop" / "naabu" / "evidence" / "note.md"
    ).read_text(encoding="utf-8")
    ...
    braa_meta = (ROOT / "fixtures" / "pack_drop" / "braa" / "meta.json").read_text(
        encoding="utf-8"
    )
    braa_note = (
        ROOT / "fixtures" / "pack_drop" / "braa" / "evidence" / "note.md"
    ).read_text(encoding="utf-8")
