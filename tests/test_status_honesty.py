...
COVEY_PACK_HEAD = "ae9ec27c"
STALE_PACK_HEAD = "1b365957"
STALE_PACK_HONESTY = "53233649"
COVEY_E2E_UNPROVEN = (
    "masscan",
    "arp-scan",
    "netdiscover",
...
def test_status_next_action_is_reid_only_blockers() -> None:
    ...
    low = action.lower()
    assert "cos #33" in low
    assert "honesty sync" in low
    assert "cos32-pack-drop-braa" in low
    assert status.get("item") == "COS33-PACK-DROP-IKE-SCAN"
    assert "next brick" in low and "ike-scan" in low
    ...
    # Pack HEAD ae9ec27c (PR #55). Covey HEAD still 30d2197f pack_drop
    # export. Pack STATUS must not restamp CoS #32 / pack 1b365957 / 53233649.
...
def _live_this_window(text: str) -> str:
    """Current-cycle window / newest delta — not historical cycle-139 notes."""
    for needle in (
        "**This window",
        "**Delta (cycle 140):",
        "**Delta (cycle 139):",
...
def test_status_and_plan_cannot_lag_covey_e2e_set() -> None:
    ...
    for where, text in (("STATUS next_action", action), ("PLAN this-window", window)):
        ...
        assert "cos #33" in low, f"{where} missing CoS #33 stamp"
        assert "next brick" in low, f"{where} missing next brick = ike-scan"
        assert "ike-scan" in low, f"{where} missing next brick = ike-scan"
        assert "cos32-pack-drop-braa" in low, f"{where} missing COS32-PACK-DROP-BRAA DONE"
        assert "stop for cos #34" in low, f"{where} missing Stop for CoS #34"
...
def test_status_and_live_docs_match_cos33_covey_e2e_proven() -> None:
    """Pack next_action / this-window docs follow CoS #33 pack HEAD E2E_PROVEN."""
    status = _status()
    action = status.get("next_action", "")
    low = action.lower()
    assert status.get("paying_day") == "FAIL"
    ...
    for path in live:
        ...
        if not window:
            ...
            if path.name == "PLAN.md":
                ...
                idx = text.find("CoS #33")
                window = text[idx : idx + 1600] if idx >= 0 else ""
        assert window, f"{path} missing CoS #33 this-window copy"
        win_low = window.lower()
        assert "cos #33" in win_low, f"{path} this-window is not CoS #33"
