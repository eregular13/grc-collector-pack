...
COVEY_PACK_HEAD = "1b365957"
STALE_PACK_HEAD = "7c7ef17c"
STALE_PACK_HONESTY = "adc479e3"
COVEY_E2E_UNPROVEN = (
    "masscan",
    "arp-scan",
    "netdiscover",
...
def test_status_next_action_is_reid_only_blockers() -> None:
    ...
    low = action.lower()
    assert "cos #32" in low
    assert "honesty sync" in low
    assert "cos31-pack-drop-nbtscan" in low
    assert status.get("item") == "COS32-PACK-DROP-BRAA"
    assert "next brick" in low and "braa" in low
    ...
    # Pack HEAD 1b365957 (PR #53). Covey HEAD still 30d2197f pack_drop
    # export. Pack STATUS must not restamp CoS #31 / pack 7c7ef17c / adc479e3.
...
def _live_this_window(text: str) -> str:
    """Current-cycle window / newest delta — not historical cycle-137 notes."""
    for needle in (
        "**This window",
        "**Delta (cycle 138):",
        "**Delta (cycle 137):",
...
def test_status_and_plan_cannot_lag_covey_e2e_set() -> None:
    ...
    for where, text in (("STATUS next_action", action), ("PLAN this-window", window)):
        ...
        assert "cos #32" in low, f"{where} missing CoS #32 stamp"
        assert "next brick" in low, f"{where} missing next brick = braa"
...
def test_status_and_live_docs_match_cos32_covey_e2e_proven() -> None:
    """Pack next_action / this-window docs follow CoS #32 pack HEAD E2E_PROVEN."""
    status = _status()
    action = status.get("next_action", "")
    low = action.lower()
    assert status.get("paying_day") == "FAIL"
    ...
    for path in live:
        ...
        if not window:
            ...
            if path.name == "PLAN.md":
                ...
                idx = text.find("CoS #32")
                window = text[idx : idx + 1600] if idx >= 0 else ""
        assert window, f"{path} missing CoS #32 this-window copy"
        win_low = window.lower()
        assert "cos #32" in win_low, f"{path} this-window is not CoS #32"
