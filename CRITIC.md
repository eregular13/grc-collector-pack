...
Lab this brick: pytest pending. Ten collectors + `grc_loader` +
`tests/lab_outputs.py` pending. Zero P0/P1. MCP `lab_drop` success
JSON advertises `console_cli_twin` / `console_hint`
(`OUT_DIR=<work>/out python -m product`; Windows `set OUT_DIR=...` /
`python -m product`). Bind `127.0.0.1`. Never POSTs `/api/risks`.
`client` stays false. Operator twin remains `lab_drop_to_sor`.
Console twin is that prove `out/` (Active out/ / `PROVE_WORK_ROOT`).
Docs lock operator scripts ↔ MCP ↔ console as the same LAB dest_in
rails. No Makefile first-line. No SaaS. No RiskReady POST. Bricks
A–D intact. Catalog **unchanged** **111 / 32 / 30 / 81**. paying_day
