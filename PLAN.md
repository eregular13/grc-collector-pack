...
## This window (2026-09-19)

Cycle **168**: CLIENT-READY-HONESTY — stamp live docs to pack HEAD `34a32a84` / Covey HEAD `f1432918`. Product path LIVE on master: SAMPLE keep-lab → CISO Assistant CSVs → OpenGRC/Probo file sinks; conductor MCP `keep_status` + `keep_ciso`. Product items DONE: SAMPLE keep→CISO (PR #85), MCP keep tools (PR #83), OpenGRC+Probo exporters (PR #84). Covey `f1432918` is file-true CISO/OpenGRC/Probo leave-behinds on Covey main — pack does not own Covey adapters. Meta source lock (PR #81 / `7f6fd90d`) already on master as history — do not reopen. Integrity city PARKED forever unless Reid unlocks. STATUS `next_action` is the CLIENT-READY operator path under SAMPLE honesty until real KEEP. Reid-only blockers remain CTA, real KEEP `in/` drop 0/4, Eval `npm start`, DESKTOP Docker compose on a real host (this VM `compose_lab` absent ≠ PASS). Pytest locks STATUS `next_action` and PLAN this-window to the product-slice HEADs and the CLIENT-READY operator path. Cycle **167** SAMPLE keep-lab sinks stand as history. **Stop vanity Layer C parsers.** keep-lab uses redacted samples until Reid drops real KEEP into pack `in/` (0/4 real still open). Remaining work is Reid-only — not catalog inflation. File-drop remains the default. LICENSE-LOCK / BloodHound / Nuclei-class never `will_run=true`. File-drop-only names never `live_ready`. `FARM_TOOL_BIN` never resolves locked scanners. `SCOPE.example.yaml` does not default-allowlist nmap/nessus. nmap/nessus invoke only when SCOPE.allow_tools + stage + PATH + HITL. No slot inflation. No live probes. No fake compose pass. Paying-day stays FAIL. Compose ABSENT until proven on a Docker host. Wrap dead forever (review-only; no login/POST). DEMO ≠ client. SAMPLE ≠ client KEEP. RiskReady stay-out. pytest greens are not a paying-day stamp. Hexstrike pattern-only. MCP stub is not USB `evergreen_assessment_mcp` (`dropbox.mcp_stub` = conductor only). `argus_pack_truth` stays `evergreen_assessment_mcp` only.
