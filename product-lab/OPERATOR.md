...
| `in/wazuh/` | Wazuh agents/alerts, osquery inventory + failing checks, Fleet hosts/policies (fail only; disk encryption / MDM Off map to POA&M; empty invents nothing), Lynis report / `report.dat`, CIS-CAT/XCCDF JSON (fail only), Intune/Jamf device inventory (also `in/mdm/`; encryption % / missing EDR / unenrolled as assessment findings). Parse-only — no osqueryi / CIS-CAT / Graph / Jamf. |
