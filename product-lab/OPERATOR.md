...
| `in/vuln/` | Nuclei JSON/JSONL (`results` wrapper OK; INFO silent; no nuclei run), Nikto text/XML/JSON (interesting/high only; empty invents nothing; no nikto run / no live HTTP), Trivy, Greenbone, testssl JSON (HIGH/WARN only; no live TLS) |
