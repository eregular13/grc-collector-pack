...
| `in/wazuh/` | Wazuh agents/alerts, osquery inventory + failing checks, Fleet hosts/policies (fail only; disk encryption / MDM Off map to POA&M; empty invents nothing), Lynis report / `report.dat` (mapped warnings + suggestions only; hardening index is a score, not a finding), OpenSCAP XCCDF/ARF `--results` XML (fail/error only; SSG/ANSSI/STIG — not a CIS benchmark / CIS-CAT deliverable), CIS-CAT/XCCDF JSON (fail only), Intune/Jamf device inventory (also `in/mdm/`; encryption % / missing EDR / unenrolled as assessment findings). Parse-only — no osqueryi / CIS-CAT / oscap / Graph / Jamf. LAB dest_in rows stay LAB and never enter KEEP. |
| `in/identity/` | BloodHound CE / SharpHound JSON (`data.nodes`/`data.edges` or `Properties`/`ObjectIdentifier`/`Aces`; empty data invents nothing), PingCastle XML, CIS-CAT/XCCDF XML/JSON (fail only), HardeningKitty Audit CSV (Failed/`TestResult=Failed` only; official HK columns `ID,Category,Name,Severity,Result,Recommended,TestResult,SeverityFinding,DefaultValue,Filter`; Result=measured; host from `hardeningkitty-<HOSTNAME>-<timestamp>.csv` / sidecar / `HARDENINGKITTY_HOST`; MS Security Baseline — not CIS Benchmark / CIS-CAT; does not invent Windows findings; LAB dest_in rows stay LAB and never enter KEEP), enum4linux-ng JSON/text (`target` + users/groups/shares; null session / writable shares / Domain Admins hints map to existing identity/SMB POA&M when shown; empty invents nothing; detect does not steal HK/BloodHound; no enum4linux run / no credentials) |
...
- POA&M draft: `out/poam/poam.csv` — **Pentera finds it; Evergreen maps it.** High/critical (and key medium such as SMB/RDP exposure) get CISA CPG + NIST CSF stamps and a recommended fix. Owner and due stay blank for a human. Do not invent dates or CVEs. The loopback console POA&M tab shows open + severity + blank-owner/due counts and flags those empty cells for triage before CISO import. Coverage tab shows `applied_controls.csv` + semicolon `risk_scenarios.csv` and a `framework_refs` count heatmap (`/api/controls` `/api/scenarios` `/api/coverage`); evidence rows add `out/evidence` path/size when present.
- RiskReady: **LICENSE-LOCK stay-out**. Not generated. `push_riskready.sh` never logs in or POSTs, even if `RISKREADY_PUSH=1`. Open risks are `out/poam/poam.csv` (count identity).
- Packaged copy: `product-lab/drop/` plus `/export.zip` from the console. See `product-lab/drop/MANIFEST`. OpenGRC Data Manager CSVs (`product-lab/drop/opengrc/`) and Probo drafts (`product-lab/drop/import_preview/probo.json`) are file-true leave-behind, posted=false, not live import. Do not POST `/api/risks`.
...
CISO prints the clica path and dry-run. RiskReady prints LICENSE-LOCK stay-out (no files written) and exits 0.
