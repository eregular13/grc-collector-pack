...
| `in/vuln/` | Nuclei JSON/JSONL (`results` wrapper OK; INFO silent; no nuclei run), Nikto text/XML/JSON (interesting/high only; empty invents nothing; no nikto run / no live HTTP), Nessus `.nessus` / NessusClientData XML (`ReportHost`/`ReportItem`; High/Critical + key Medium; empty/DEMO stub invent nothing; no Nessus API / no collector invoke), Trivy, Greenbone, testssl JSON (HIGH/WARN only; no live TLS) |
