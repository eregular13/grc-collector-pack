...
| `in/nmap/` | Nmap XML, gnmap (`-oG`), thin JSON, masscan `-oX` / `-oJ` (open ports only; empty invents nothing; no nmap/masscan run), rustscan / naabu JSON/JSONL (`{ip, port}` or `{ip, ports:[int]}`; open only; empty invents nothing; no rustscan/naabu run), arp-scan text/JSON (IP + MAC + vendor; hosts become assets only; empty invents nothing; no arp-scan run / no live ARP). masscan stays file_drop / use_dont_ship; rustscan/naabu invoke stays BYO; arp-scan stays file_drop. |
