...
| EX-PUSH-DRY | `push_ciso.sh` never POST `/api/risks`; `push_riskready.sh` is review-only even if `RISKREADY_PUSH=1` | LICENSE-LOCK stay-out. RiskReady JSON is not generated. Open risks are the POA&M. |
