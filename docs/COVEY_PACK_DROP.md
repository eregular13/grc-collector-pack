...
**sslscan**). Other stdout-class adapters (tlsx, …) use the same files
when dropped here — no 17th live adapter, no pack Covey adapter work.
...
plus `fixtures/pack_drop/httpx/`, `fixtures/pack_drop/unicornscan/`, and
stdout/XML-class `fixtures/pack_drop/sslscan/`
