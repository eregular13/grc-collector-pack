...
| Covey | `in/nmap/` | pack_drop `assets.jsonl` + `findings.jsonl` + `meta.json` + `evidence/` (nmap + rustscan + httpx + unicornscan + sslscan + tlsx + whatweb + hping3 stdout/XML-class; hping3 host-only ICMP; also gnmap/XML/…) | exists — see [COVEY_PACK_DROP.md](COVEY_PACK_DROP.md) |
