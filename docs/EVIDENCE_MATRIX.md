...
| Covey | `in/nmap/` | pack_drop `assets.jsonl` + `findings.jsonl` + `meta.json` + `evidence/` (nmap + rustscan + httpx + unicornscan + sslscan + tlsx + whatweb + hping3 + onesixtyone + fping stdout/XML-class; hping3/fping host-only ICMP; onesixtyone SNMP community/sysDescr; also gnmap/XML/…) | exists — see [COVEY_PACK_DROP.md](COVEY_PACK_DROP.md) |
