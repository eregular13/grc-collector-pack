...
| Covey | `in/nmap/` | pack_drop `assets.jsonl` + `findings.jsonl` + `meta.json` + `evidence/` (nmap + rustscan + httpx + unicornscan + sslscan + tlsx + whatweb + hping3 + onesixtyone + fping + naabu + nping + nbtscan + braa + ike-scan stdout/XML-class; hping3/fping host-only ICMP; onesixtyone SNMP community/sysDescr; braa SNMP GET OID/sysDescr/sysName host-only; nbtscan NetBIOS name-table host-only; ike-scan IKE/VPN handshake host-only; naabu/nping port/service open_port_observed; also gnmap/XML/…) | exists — see [COVEY_PACK_DROP.md](COVEY_PACK_DROP.md) |
