...
- Honesty restamp: pack HEAD `ed8b381` (PR #99 cold SAMPLE→SoR fail-closed already on master) / farm HEAD `20e4f8c0` (PR #24 SAMPLE client-day dry already on main). Eval HEAD `e04c2d88` (PR #5 Node 20/22 LTS + pack sample_to_sor). Schema seam CLOSED. Integrity PARKED.
- Honesty restamp (historical): pack HEAD `9a872ef5` (PR #91 `sample_to_sor` already on master) / farm HEAD `c012dd24` (PR #23 unit-only GHA CI already on main). Eval HEAD `ebaa9f50` (PR #4). Schema seam CLOSED. Integrity PARKED.
...
- CI runs SAMPLE keep-lab as a subprocess. Honesty lock follows live pack HEAD `ed8b381`.
