...
- RETIRE_RISKREADY_JSON: `grc_loader` no longer writes `out/riskready/`. `summary.json` drops `incidents` / `risks_proposed` (outside count identity). The loopback console reads POA&M / CISO register for open-risk KPIs and `/api/proposed`. `out/simplerisk/poam.csv` carries the estate banner (`#` comment) plus the per-row `estate` column (PR #132 estate-banner work is still open on master; this sink is covered here). Never POSTs `/api/risks`. Does not touch `product-lab/drop`.
