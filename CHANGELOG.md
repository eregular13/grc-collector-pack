...
- EXEC_AND_TRUST_PAGES: every named export (executive summary, `SCOPE_AND_TRUST.md`, `poam.csv`/`poam.md`, CISO CSVs, OpenGRC, Probo) opens with the Argus Part 0 estate banner. CSVs also carry an `estate` column. Exactly one allowed label; fail closed to the most restrictive when unsure. SAMPLE/DEMO/LAB and any `product-lab/drop` fallback cannot become CLIENT and cannot be suppressed (MIXED when some outputs fell back). One-page exec summary + `SCOPE_AND_TRUST.md` are generated from the run's own files; missing values print `not recorded`; reviewer narrative slots stay marked placeholders. Internal cycle/CoS/adapter notes stay off client pages. RiskReady stay-out. No POST `/api/risks`.
