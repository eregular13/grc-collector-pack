...
- RETIRE_RISKREADY_JSON: `grc_loader` no longer writes `out/riskready/`. `summary.json` drops `incidents` / `risks_proposed` (outside count identity). The loopback console reads POA&M / CISO register for open-risk KPIs and `/api/proposed`. `out/simplerisk/poam.csv` starts with the exact header (no `#` preamble), carries the per-row `estate` column, and has sidecar `out/simplerisk/ESTATE.txt` (aligned with the #132 CSV-import revision). Never POSTs `/api/risks`. Does not touch `product-lab/drop`.
