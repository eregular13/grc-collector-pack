...
BRAKES_DEFAULTS = (
    ...
    ("byo_path", "allow_tools ∩ PATH; missing → plan-only; never apt/embed; FARM_TOOL_BIN never resolves LICENSE-LOCK"),
    ("free_day_scope", "SCOPE.example.yaml host-local; nmap/nessus not default"),
    ("pack_truth", "evergreen_assessment_mcp check_scope/license_guard; mcp_stub is conductor UX"),
...
def render_slots_md() -> str:
    ...
    lines.extend(
        [
            ...
            "subprocess; no live ARP). Also drop **nbtscan** name/IP tables",
            ...
            "bloodhound / azurehound stay file_drop (never subprocess).",
            "",
            "Drop **enum4linux-ng** JSON or text under `in/identity/` (`target` +",
            "`users`/`groups`/`shares`, or `Share Enumeration` text). Users/groups/shares",
            "already listed are preferred. Null session, writable shares, and Domain",
            "Admins / Backup Operators hints map to existing identity/SMB POA&M only when",
            "the export already shows them. Empty invents nothing. Detect does not steal",
            "HardeningKitty CSV or BloodHound JSON. Parse-only — no enum4linux run, no",
            "credentials. Empty `in/` still loads `fixtures/demo/identity/enum4linux-ng.txt`.",
            "enum4linux-ng stays `file_drop`. No new catalog slots.",
            "",
            ...
            "Windows findings). LAB dest_in uses Microsoft Security Baseline lists",
            ...
            "",
            "Drop **Intune** / **Jamf** device inventory JSON or CSV under `in/mdm/`",
            "(host-wazuh extra path) or `in/wazuh/`. Graph `managedDevices`, Jamf",
            "`computers`/`computer`, or a device CSV with encryption / EDR / MDM columns.",
            "Encryption compliance below 100%, missing EDR, and unenrolled devices map",
            "to CISO/POA&M as assessment findings (not breaches). Fleet/Wazuh shapes are",
            "not stolen. Empty `value` / `computers` invent nothing. Parse-only — no",
            "Graph, Intune, Jamf, or osqueryi. Empty `in/` still loads",
            "`fixtures/demo/mdm/intune-devices.json` and `jamf-computers.json`.",
            "osqueryi / wazuh-agent stay file_drop. No new catalog slots.",
            "",
            ...
            "Also drop **Entra / Okta / Google user-inventory** JSON or CSV under",
            "`in/saas/` (`users` / `@odata.context` users / `primaryEmail` CSV).",
            "Explicit MFA-not-registered, standing Global Administrator, and stale",
            "guest rows become assessment findings (not breaches). Missing MFA fields",
            "invent nothing. High MFA / standing Global Administrator / stale guest",
            "rows map to CISO/POA&M.",
            ...
            "`okta-users.json`, `entra-users.json`, `google-users.csv`,",
