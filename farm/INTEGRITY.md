...
| `farm_toolbin_status` | Resolve + SCOPE honesty | Per-slot `live_ready`. `will_run` may be true for DEMO lab stubs; `live_ready` stays 0 on DEMO SCOPE, stubs, or file_drop-only names even if a binary is under `FARM_TOOL_BIN`. Real `--live` needs signed non-DEMO SCOPE + allowlisted real binary |
...
| SimpleRisk | `out/simplerisk/` leave-behind | POA&M copy under `out/` only. No API. No push. Human copies rows. CISO Assistant (clica/UI) is SoR |
